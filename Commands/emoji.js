...
function emojis(message) {
    ...
    if (["emoji", 'setemoji', 'economyemoji', 'currency', 'setcurrency'].includes(command)) {
        ...
        if (emoji == 'reset') {
            ...
            return message.channel.send(functions.embed(`Successfully reset currency`, `Reset currency to ${get(message.guild)}`, "#0099ff"))
        }
        if (!emoji) {
            return message.channel.send(functions.embed(`Currency for ${message.guild.name}`, `The currency for ${message.guild.name} is ${get(message.guild)}\n\n**Change it with ${prefix.get(message.guild)}currency [currency to set]**!`, "#0099ff"))}
        ...
        message.channel.send(functions.embed(`Successfully set currency`, `Set currency for ${message.guild.name} to ${score}`, "#0099ff"))
    }
}
function get(guild) {
    setup.emojis(guild)
